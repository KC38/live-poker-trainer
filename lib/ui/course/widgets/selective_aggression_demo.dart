...
class SelectiveAggressionDemo extends StatefulWidget {
  ...
  @override
  State<SelectiveAggressionDemo> createState() => _SelectiveAggressionDemoState();
}
...
class _SelectiveAggressionDemoState extends State<SelectiveAggressionDemo> {
  ...
  ({String label, String caption, Color color})? get _nextPoint {
    for (final point in SelectiveAggressionDemo.points) {
  ...
  Widget build(BuildContext context) {
    ...
    // sparse green under TIGHT / BARREL / SAMPLE).
    final feltHeight =
          ...
          Expanded(
            child: _SelectiveSoftPulse(
              active:
                  widget.interactive &&
                  widget.enabled &&
                  next?.label == SelectiveAggressionDemo.points[i].label,
              child: _SelectiveAggressionTile(
                ...
                densify: expandTeach,
                selected: _tapped.contains(SelectiveAggressionDemo.points[i].label),
                enabled: widget.interactive && widget.enabled,
                onPressed: widget.interactive
                    ? () => _onTap(SelectiveAggressionDemo.points[i].label)
                    : null,
              ),
            ),
          ),
...
class _SelectiveSoftPulse extends StatefulWidget {
  const _SelectiveSoftPulse({required this.active, required this.child});

  final bool active;
  final Widget child;

  @override
  State<_SelectiveSoftPulse> createState() => _SelectiveSoftPulseState();
}

class _SelectiveSoftPulseState extends State<_SelectiveSoftPulse>
  ...
  @override
  void didUpdateWidget(covariant _SelectiveSoftPulse oldWidget) {
