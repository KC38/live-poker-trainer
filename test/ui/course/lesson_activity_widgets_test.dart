...
void main() {
  ...
  test('resolveCoachDialogueVisual is content-driven', () {
    ...
    // Bare "call"/"fold" phrases must not steal flop-lines from passive/vs-open.
    expect(
      resolveCoachDialogueVisual(
        CourseActivity(
          id: 'act-generic-passive',
          order: 1,
          stage: ActivityStage.explain,
          renderer: ActivityRenderer.coachDialogue,
          estimatedSeconds: 30,
          accessibilityText: 'Fold ends. Check passes. Call matches.',
          acceptedGrades: const [SoftGrade.recommended],
        ),
      ).kind,
  ...
  testWidgets(
    'flop-lines explain taps six line tiles instead of Continue',
    (tester) async {
    final activity = CourseActivity(
      id: 'act-03-04-01-explain',
      order: 1,
    ...
    expect(
      find.text('Tap Value, C-Bet, Check, Call, Fold, and Raise.'),
      findsOneWidget,
    );
    expect(resolveCoachDialogueVisual(activity).requiresFeltTap, isTrue);
    expect(isTableRegionTapActivity(activity), isTrue);
