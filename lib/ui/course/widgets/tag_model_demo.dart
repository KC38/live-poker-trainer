...
class TagModelDemo extends StatefulWidget {
  ...
  static const points = <({String label, String caption, Color color})>[
    (label: 'TIGHT', caption: 'Tight in', color: AppColors.gold),
    (label: 'AGGRO', caption: 'Aggressive after', color: AppColors.cream),
    (label: 'MODEL', caption: 'Working model', color: AppColors.danger),
  ];

  @override
  State<TagModelDemo> createState() => _TagModelDemoState();
}

class _TagModelDemoState extends State<TagModelDemo> {
  ...
  ({String label, String caption, Color color})? get _nextPoint {
    for (final point in TagModelDemo.points) {
  ...
  Widget build(BuildContext context) {
    ...
    // sparse green under TIGHT / AGGRO / MODEL).
    final feltHeight =
          ...
          Expanded(
            child: _TagSoftPulse(
              active:
                  widget.interactive &&
                  widget.enabled &&
                  next?.label == TagModelDemo.points[i].label,
              child: _TagModelTile(
                label: TagModelDemo.points[i].label,
                caption: TagModelDemo.points[i].caption,
                color: TagModelDemo.points[i].color,
                densify: expandTeach,
                selected: _tapped.contains(TagModelDemo.points[i].label),
                enabled: widget.interactive && widget.enabled,
                onPressed: widget.interactive
                    ? () => _onTap(TagModelDemo.points[i].label)
                    : null,
              ),
            ),
          ),
    ...
    final cue =
        !showCue
            ...
            : Container(
              ...
              child: Text(
                'Tight in, aggressive after — a working model',
...
class _TagModelTile extends StatelessWidget {
  const _TagModelTile({
    required this.label,
...
class _TagSoftPulse extends StatefulWidget {
  const _TagSoftPulse({required this.active, required this.child});

  final bool active;
  final Widget child;

  @override
  State<_TagSoftPulse> createState() => _TagSoftPulseState();
}

class _TagSoftPulseState extends State<_TagSoftPulse>
  ...
  @override
  void didUpdateWidget(covariant _TagSoftPulse oldWidget) {
