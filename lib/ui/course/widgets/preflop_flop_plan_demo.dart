...
class _PreflopFlopPlanDemoState extends State<PreflopFlopPlanDemo> {
  ...
  ({String label, String caption, Color color})? get _nextPoint {
    for (final point in PreflopFlopPlanDemo.points) {
  ...
  Widget build(BuildContext context) {
    ...
    // sparse green under REASON / CONFIRM / CANCEL).
    final feltHeight =
    ...
    final tiles = Row(
      ...
      children: [
        for (var i = 0; i < PreflopFlopPlanDemo.points.length; i++) ...[
          ...
          Expanded(
            child: _PreflopFlopPlanSoftPulse(
              active:
                  widget.interactive &&
                  widget.enabled &&
                  next?.label == PreflopFlopPlanDemo.points[i].label,
              child: _PreflopFlopPlanTile(
                ...
                densify: expandTeach,
                selected: _tapped.contains(PreflopFlopPlanDemo.points[i].label),
...
class _PreflopFlopPlanSoftPulse extends StatefulWidget {
  const _PreflopFlopPlanSoftPulse({required this.active, required this.child});

  final bool active;
  final Widget child;

  @override
  State<_PreflopFlopPlanSoftPulse> createState() => _PreflopFlopPlanSoftPulseState();
}

class _PreflopFlopPlanSoftPulseState extends State<_PreflopFlopPlanSoftPulse>
  ...
  @override
  void didUpdateWidget(covariant _PreflopFlopPlanSoftPulse oldWidget) {
