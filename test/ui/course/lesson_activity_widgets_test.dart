...
void main() {
  ...
  testWidgets(
    'session-guardrails explain taps Quit Guard Stop instead of Continue',
    (tester) async {
      final activity = CourseActivity(
      ...
      expect(find.byType(SessionGuardrailsDemo), findsOneWidget);
      expect(find.text('Tap Quit, Guard, and Stop.'), findsOneWidget);
      expect(resolveCoachDialogueVisual(activity).requiresFeltTap, isTrue);
      expect(isTableRegionTapActivity(activity), isTrue);

      for (final title in ['QUIT', 'GUARD', 'STOP']) {
        await tester.tap(find.text(title));
        await tester.pump();
      }
      expect(feltAck, 1);
