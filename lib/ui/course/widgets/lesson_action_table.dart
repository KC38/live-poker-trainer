...
/// Value / hang / ego tiles for versus-maniac explain demos.
class VsManiacDemo extends StatefulWidget {
  /// Creates the demo.
  const VsManiacDemo({
    super.key,
    this.interactive = false,
  ...
  static const points = <({String label, String caption, Color color})>[
    (label: 'VALUE', caption: 'Call wider for value', color: AppColors.gold),
    (label: 'HANG', caption: 'Let them overplay', color: AppColors.cream),
    (label: 'EGO', caption: 'Leave it at home', color: AppColors.danger),
  ];

  @override
  State<VsManiacDemo> createState() => _VsManiacDemoState();
}

class _VsManiacDemoState extends State<VsManiacDemo> {
  final Set<String> _tapped = <String>{};

  void _onTap(String label) {
    if (!widget.enabled || widget.onAllPointsTapped == null) return;
    setState(() => _tapped.add(label));
    if (_tapped.length >= VsManiacDemo.points.length) {
      widget.onAllPointsTapped!();
    }
  }
    ...
    final child = Container(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              for (var i = 0; i < VsManiacDemo.points.length; i++) ...[
                if (i > 0) const SizedBox(width: 8),
                Expanded(
                  child: _DemoActionCard(
                    label: VsManiacDemo.points[i].label,
                    caption: VsManiacDemo.points[i].caption,
                    color: VsManiacDemo.points[i].color,
                    selected: _tapped.contains(VsManiacDemo.points[i].label),
                    enabled: widget.interactive && widget.enabled,
                    onPressed:
                        widget.interactive
                            ? () => _onTap(VsManiacDemo.points[i].label)
                            : null,
                  ),
                ),
              ],
