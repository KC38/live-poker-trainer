...
/// Explain-step demo: flop lines (value / c-bet / check / call / fold / raise).
class FlopLinesDemo extends StatefulWidget {
  ...
  static const points = <({String label, String caption, Color color})>[
    (label: 'VALUE', caption: 'Strong made', color: AppColors.gold),
    (label: 'C-BET', caption: 'Continue', color: AppColors.cream),
    (label: 'CHECK', caption: 'Back / give up', color: AppColors.slate),
    (label: 'CALL', caption: 'Continue cheap', color: AppColors.cream),
    (label: 'FOLD', caption: 'Give up', color: AppColors.danger),
    (label: 'RAISE', caption: 'Reopen', color: AppColors.gold),
  ];
...
class _FlopLinesDemoState extends State<FlopLinesDemo> {
  ...
  void _onTap(String label) {
    if (!widget.enabled || widget.onAllPointsTapped == null) return;
    setState(() => _tapped.add(label));
    if (_tapped.length >= FlopLinesDemo.points.length) {
      widget.onAllPointsTapped!();
    }
  }
            ...
            Row(
              children: [
                for (var col = 0; col < 3; col++) ...[
                  ...
                  Expanded(
                    child: _DemoActionCard(
                      label: FlopLinesDemo.points[row * 3 + col].label,
                      caption: FlopLinesDemo.points[row * 3 + col].caption,
                      color: FlopLinesDemo.points[row * 3 + col].color,
                      selected: _tapped.contains(
                        FlopLinesDemo.points[row * 3 + col].label,
                      ),
                      enabled: widget.interactive && widget.enabled,
                      onPressed:
                          widget.interactive
