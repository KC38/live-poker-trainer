...
class UserStatsModel {
  ...
  Map<String, Object?> toFirestoreMap() => {
    ...
    'streetAccuracy': {
      for (final e in streetAccuracy.entries)
        e.key: {'played': e.value.played, 'correct': e.value.correct},
    },
  ...
  factory UserStatsModel.fromFirestoreMap(Map<String, dynamic> data) {
    ...
    return UserStatsModel(
      totalSpots:
          (data['decisionsReviewed'] as num?)?.toInt() ??
          (data['totalSpots'] as num?)?.toInt() ??
          0,
      correctSpots:
          (data['recommendedOrStrong'] as num?)?.toInt() ??
          (data['correctSpots'] as num?)?.toInt() ??
          0,
      netEvBb:
          (data['netResultBb'] as num?)?.toDouble() ??
          (data['netEvBb'] as num?)?.toDouble() ??
          0,
      archetypeAccuracy:
          archRaw is Map
              ? archRaw.map((k, v) {
...
@immutable
class StreetStat {
  const StreetStat({required this.street, this.played = 0, this.correct = 0});
