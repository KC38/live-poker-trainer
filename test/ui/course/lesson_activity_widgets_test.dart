...
void main() {
  ...
  testWidgets('vs-maniac explain taps Value Hang Ego instead of Continue', (
    tester,
  ) async {
    final activity = CourseActivity(
      id: 'act-04-08-03-explain',
    ...
    expect(find.byType(VsManiacDemo), findsOneWidget);
    expect(find.text('Tap Value, Hang, and Ego.'), findsOneWidget);
    expect(resolveCoachDialogueVisual(activity).requiresFeltTap, isTrue);
    expect(isTableRegionTapActivity(activity), isTrue);

    for (final title in ['VALUE', 'HANG', 'EGO']) {
      await tester.tap(find.text(title));
      await tester.pump();
    }
    expect(feltAck, 1);
