...
class _OverbetGeometryDemoState extends State<OverbetGeometryDemo> {
  ...
  ({String label, String caption, Color color})? get _nextPoint {
    for (final point in OverbetGeometryDemo.points) {
  ...
  Widget build(BuildContext context) {
    ...
    // sparse green under OVERBET / POLAR / GEO).
    final feltHeight =
    ...
    final tiles = Row(
      ...
      children: [
        for (var i = 0; i < OverbetGeometryDemo.points.length; i++) ...[
          ...
          Expanded(
            child: _OverbetSoftPulse(
              active:
                  widget.interactive &&
                  widget.enabled &&
                  next?.label == OverbetGeometryDemo.points[i].label,
              child: _OverbetTile(
  ...
  Widget build(BuildContext context) {
    ...
    final card = AnimatedContainer(
      ...
      child: Column(
        ...
        children: [
          // FittedBox keeps OVERBET on one line in densified SoftPulse lanes.
          FittedBox(
...
class _OverbetSoftPulse extends StatefulWidget {
  const _OverbetSoftPulse({required this.active, required this.child});

  final bool active;
  final Widget child;

  @override
  State<_OverbetSoftPulse> createState() => _OverbetSoftPulseState();
}

class _OverbetSoftPulseState extends State<_OverbetSoftPulse>
  ...
  @override
  void didUpdateWidget(covariant _OverbetSoftPulse oldWidget) {
