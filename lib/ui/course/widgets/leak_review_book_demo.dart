...
/// Leak / book / review tiles for leak-review-book explain demos.
...
class _LeakReviewBookDemoState extends State<LeakReviewBookDemo> {
  ...
  Widget build(BuildContext context) {
    ...
    // sparse green under LEAK / BOOK / REVIEW).
    final feltHeight =
    ...
    final tiles = Row(
      ...
      children: [
        for (var i = 0; i < LeakReviewBookDemo.points.length; i++) ...[
          ...
          Expanded(
            child: _LeakReviewBookSoftPulse(
              active:
                  widget.interactive &&
                  widget.enabled &&
              ...
              child: _LeakReviewBookTile(
                ...
                densify: expandTeach,
                selected: _tapped.contains(LeakReviewBookDemo.points[i].label),
...
class _LeakReviewBookSoftPulse extends StatefulWidget {
  const _LeakReviewBookSoftPulse({required this.active, required this.child});

  final bool active;
  final Widget child;

  @override
  State<_LeakReviewBookSoftPulse> createState() => _LeakReviewBookSoftPulseState();
}

class _LeakReviewBookSoftPulseState extends State<_LeakReviewBookSoftPulse>
  ...
  @override
  void didUpdateWidget(covariant _LeakReviewBookSoftPulse oldWidget) {
